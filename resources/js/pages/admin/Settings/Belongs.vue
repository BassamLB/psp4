...
function performDelete() {
  ...
  router.delete(`/admin/settings/belongs/${deletingId.value}`);
  cancelDelete();
}
...
async function submitCreate() {
  await createForm.post('/admin/settings/belongs', {
    onSuccess: () => {
...
async function submitEdit() {
  ...
  await editForm.put(`/admin/settings/belongs/${editItem.value.id}`, {
    onSuccess: () => {
    ...
    <div class="space-y-4 p-4">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-lg font-semibold leading-tight">الانتماءات</h1>
            <p class="text-xs text-slate-500">إدارة الانتماءات — أضف أو حرّر أو احذف الانتماءات.</p>
          </div>
        </div>
      ...
      <div v-if="!hasItems" class="p-6 bg-background rounded text-center text-sm text-gray-600 dark:text-gray-400">لا توجد عناصر بعد. انقر "جديد" لإضافة انتماء.</div>

      <transition-group appear name="settle" tag="ul" v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <li v-for="(item, index) in filtered" :key="item.id" :style="{ transitionDelay: (index * 45) + 'ms' }" class="settle-item bg-background  rounded-lg p-4 shadow hover:shadow-md transform hover:-translate-y-0.5 transition-all duration-150 border border-slate-100 dark:border-slate-700 flex items-center justify-between">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full flex items-center justify-center bg-background text-slate-700 dark:text-slate-200 font-semibold">{{ item.name.charAt(0).toUpperCase() }}</div>
          ...
          <div class="flex items-center gap-2">
            <button @click="openEdit(item)" class="inline-flex items-center gap-1 text-indigo-600 dark:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-indigo-600/20 px-2 py-1 rounded"><Edit3 class="w-4 h-4" /><span class="text-sm">تعديل</span></button>
            <button @click="confirmDelete(item.id)" class="inline-flex items-center gap-1 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-600/20 px-2 py-1 rounded"><Trash2 class="w-4 h-4" /><span class="text-sm">حذف</span></button>
          </div>
        </li>
      </transition-group>
      ...
      <Dialog :open="showCreate" @update:open="showCreate = $event">
        <DialogContent class="sm:max-w-md bg-background rounded-xl shadow-xl w-11/12 max-w-md p-6 text-right border border-slate-100 dark:border-slate-700">
          <DialogHeader class="flex items-center justify-between">
            <DialogTitle>إنشاء انتماء</DialogTitle>
            <DialogClose />
          </DialogHeader>
          <DialogDescription>
            أضف انتماءً جديداً إلى النظام.
          </DialogDescription>
      ...
      <Dialog :open="showEdit" @update:open="showEdit = $event">
        <DialogContent class="sm:max-w-md bg-background rounded-xl shadow-xl w-11/12 max-w-md p-6 text-right border border-slate-100 dark:border-slate-700">
          <DialogHeader class="flex items-center justify-between">
            <DialogTitle>تعديل انتماء</DialogTitle>
            <DialogClose />
          </DialogHeader>
          <DialogDescription>
            حرّر الانتماء المختار.
          </DialogDescription>

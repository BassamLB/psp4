...
function performDelete() {
  ...
  router.delete(`/admin/settings/professions/${deletingId.value}`);
  cancelDelete();
}
...
async function submitCreate() {
  await createForm.post('/admin/settings/professions', {
    onSuccess: () => {
...
async function submitEdit() {
  ...
  await editForm.put(`/admin/settings/professions/${editItem.value.id}`, {
    onSuccess: () => {
    ...
    <div class="space-y-4 p-4">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-lg font-semibold leading-tight">المهن</h1>
            <p class="text-xs text-slate-500">إدارة المهن — أضف، حرّر أو احذف المهن.</p>
          </div>
        </div>
      ...
      <div v-if="!hasItems" class="p-6 bg-background rounded text-center text-sm text-gray-600 dark:text-gray-400">لا توجد عناصر بعد. انقر "جديد" لإضافة مهنة جديدة.</div>
      ...
      <Dialog :open="showCreate" @update:open="showCreate = $event">
        <DialogContent class="sm:max-w-md bg-background rounded-xl shadow-xl w-11/12 max-w-md p-6 text-right border border-slate-100 dark:border-slate-700">
          <DialogHeader class="flex items-center justify-between">
            <DialogTitle>إنشاء مهنة</DialogTitle>
            <DialogClose />
          </DialogHeader>
          <DialogDescription>
            أضف مهنة جديدة إلى النظام.
          </DialogDescription>
      ...
      <Dialog :open="showEdit" @update:open="showEdit = $event">
        <DialogContent class="sm:max-w-md bg-background rounded-xl shadow-xl w-11/12 max-w-md p-6 text-right border border-slate-100 dark:border-slate-700">
          <DialogHeader class="flex items-center justify-between">
            <DialogTitle>تعديل مهنة</DialogTitle>
            <DialogClose />
          </DialogHeader>
          <DialogDescription>
            حرّر المهنة المختارة.
          </DialogDescription>

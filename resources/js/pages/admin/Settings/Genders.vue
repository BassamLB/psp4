...
function performDelete() {
    ...
    router.delete(`/admin/settings/genders/${deletingId.value}`);
    cancelDelete();
}
...
async function submitCreate() {
    await createForm.post('/admin/settings/genders', {
        onSuccess: () => {
...
async function submitEdit() {
    ...
    await editForm.put(`/admin/settings/genders/${editItem.value.id}`, {
        onSuccess: () => {
...
const filtered = computed(() => {
    ...
    return list.filter((i) => {
        return String(i.name).toLowerCase().includes(q) || String(i.id).includes(q);
    });
});
</script>
        ...
        <div class="space-y-4 p-4">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-lg font-semibold leading-tight">الجنس</h1>
                        <p class="text-xs text-slate-500">إدارة أنواع الجنس — أضف، حرّر أو احذف الصفوف بسهولة.</p>
                    </div>
                </div>
            ...
            <div v-if="!hasItems" class="p-6 bg-background rounded text-center text-sm text-gray-600 dark:text-gray-400">
                لا توجد عناصر بعد. انقر "جديد" لإضافة جنس جديد.
            </div>
            ...
            <transition-group appear name="settle" tag="ul" v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <li
                    ...
                    class="settle-item bg-background rounded-lg p-4 shadow hover:shadow-md transform hover:-translate-y-0.5 transition-all duration-150 border border-slate-100 dark:border-slate-700 flex items-center justify-between"
                >
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full flex items-center justify-center bg-background text-slate-700 dark:text-slate-200 font-semibold">
                            {{ item.name.charAt(0).toUpperCase() }}
                        </div>
                        <div class="text-right">
                            <div class="font-medium text-slate-700 dark:text-white">{{ item.name }}</div>
                            <div class="text-xs text-slate-400 dark:text-slate-300">معرّف #{{ item.id }}</div>
                        </div>
                    </div>

                    <div class="flex items-center gap-2">
                        <button @click="openEdit(item)" class="inline-flex items-center gap-1 text-indigo-600 dark:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-indigo-600/20 px-2 py-1 rounded">
                            <Edit3 class="w-4 h-4" />
                            <span class="text-sm">تعديل</span>
                        </button>
                        <button @click="confirmDelete(item.id)" class="inline-flex items-center gap-1 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-600/20 px-2 py-1 rounded">
            ...
            <nav v-if="props.items.prev_page_url || props.items.next_page_url" class="flex items-center justify-center gap-4 mt-4">
                <Link
                    ...
                    class="px-3 py-1 bg-background border rounded hover:bg-gray-50 inline-flex items-center gap-2 dark:text-gray-300 dark:hover:bg-gray-600 dark:border-slate-700"
                ><ChevronLeft class="w-4 h-4" />السابق</Link>
                ...
                <Link
                    ...
                    class="px-3 py-1 bg-background border rounded hover:bg-gray-50 inline-flex items-center gap-2 dark:text-gray-300 dark:hover:bg-gray-600 dark:border-slate-700"
                >التالي<ChevronRight class="w-4 h-4" /></Link>
            </nav>
                        ...
                        <Dialog :open="showConfirm" @update:open="showConfirm = $event">
                            <DialogContent class="sm:max-w-md bg-background rounded-xl shadow-xl w-11/12 max-w-md p-6 text-right border border-slate-100 dark:border-slate-700">
                                <div class="flex items-start gap-3">
                                    <div class="flex-none rounded-full bg-red-100 p-3 text-red-600">
            ...
            <Dialog :open="showCreate" @update:open="showCreate = $event">
                <DialogContent class="sm:max-w-md bg-background rounded-xl shadow-xl w-11/12 max-w-md p-6 text-right border border-slate-100 dark:border-slate-700">
                    <DialogHeader class="flex items-center justify-between">
                        <DialogTitle>إنشاء جنس</DialogTitle>
                        <DialogClose />
                    </DialogHeader>
                    <DialogDescription>
                        أضف جنساً جديداً إلى النظام.
                    </DialogDescription>
            ...
            <Dialog :open="showEdit" @update:open="showEdit = $event">
                <DialogContent class="sm:max-w-md bg-background rounded-xl shadow-xl w-11/12 max-w-md p-6 text-right border border-slate-100 dark:border-slate-700">
                    <DialogHeader class="flex items-center justify-between">
                        <DialogTitle>تعديل جنس</DialogTitle>
                        <DialogClose />
                    </DialogHeader>
                    <DialogDescription>
                        حرّر الجنس المختار.

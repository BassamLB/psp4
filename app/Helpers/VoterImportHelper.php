...
{
    /** @var Collection<string, Gender>|null */
    private static ?Collection $genderCache = null;

    /** @var Collection<string, Town>|null */
    private static ?Collection $townCache = null;

    /** @var Collection<string, Sect>|null */
    private static ?Collection $sectCache = null;
    ...
    public static function initializeCaches(): void
    {
        ...
        self::$sectCache = Sect::all()->keyBy(function ($item) {
            return $item->name ? self::normalizeArabic($item->name) : '';
        });
    }
    ...
    public static function findDoctrineId(?string $name): ?int
    {
        ...
        if (self::$sectCache && self::$sectCache->has($normalized)) {
            return self::$sectCache->get($normalized)->id;
        }

        $sect = Sect::whereRaw('LOWER(name) = ?', [$normalized])->first();
    ...
    public static function prepareVoterData(array $row): array
    {
        return [
            ...
            'sect_id' => self::findDoctrineId($row['doctrine'] ?? null),
     ...
     */
    /**
     * @param  array<string,mixed>  $voterData
     */
     ...
     */
    /**
     * @param  array<int,array<string,mixed>>  $importedData
